...
async def fetch_route_core(
    ...
    # 4) 폴백: Manhattan 추정
    # raw_time: 기상 보정 전 이동 시간 (캐시 일관성 유지)
    # time:     기상 보정 후 이동 시간 (호출자에게 반환)
    # get_dynamic_speed에 weather_factor=1.0을 넘겨 raw 속도 기준으로 raw_time을 산출하고,
    # time은 weather_factor를 별도로 적용한다.
    # 이렇게 해야 캐시에서 로드할 때 raw_time * weather_factor 연산이 올바르게 동작한다.
    d_km   = manhattan_distance(curr["lat"], curr["lon"], dest["lat"], dest["lon"])
    raw_spd = get_dynamic_speed(dest["lat"], dest["lon"], speed, congestion_penalty, 1.0)
    raw_t  = (d_km / raw_spd) * 60 if raw_spd > 0 else 0.0
    fb = {
        "from":     curr["name"],
        "to":       dest["name"],
        "dist":     d_km,
        "raw_time": raw_t,
        "time":     raw_t * weather_factor,
        "toll":     0,
