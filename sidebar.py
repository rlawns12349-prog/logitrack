...
def _render_csv_upload(db, kakao_key: str):
    template = (
        ...
        "평균시속(km/h),45,도심 기준 45 권장\n"
        "교통혼잡도(%),40,혼잡할수록 높게 입력 (0~100)\n"
        "기본하차시간(분),10,거점당 기본 작업 시간\n"
        "kg당추가시간(초),2,무게 1kg 당 추가되는 하차 시간\n"
        "최대근로시간(시간),10,기사 1인 최대 근무 시간\n"
        "업무균등화,0,트럭 간 업무량 균등 배분 (0=사용안함 / 1=사용)\n"
        "경유단가(원/L),1500,현재 경유 단가\n"
        "인건비(원/시간),15000,기사 시간당 인건비\n"
        "최적화시간(초),5,숫자가 클수록 정확하나 느림 (5~30 권장)\n"
        "[거점]\n"
