...
def _render_loading_guide(truck_name: str, res: dict) -> None:
    ...
    with st.expander("📦 출발 전 적재 순서 안내", expanded=True):
        ...
        for order, item in enumerate(reversed(loads)):
            ...
            if is_first:
                border, bg, tag = "3px solid #3b82f6", "#eff6ff", "🔵 가장 바깥쪽 — 첫 번째 하차"
            elif is_last:
                border, bg, tag = "3px solid #6b7280", "#f9fafb", "⬛ 가장 안쪽 — 마지막 하차"
            else:
                position = total - order
                border, bg, tag = "2px solid #d1d5db", "#ffffff", f"안에서 {position}번째"

            st.markdown(f"""
<div style="border:{border};border-radius:8px;padding:10px 14px;margin-bottom:8px;background:{bg};">
  <div style="font-weight:700;font-size:1rem;">{order+1}번째로 싣기 — {item['name']}</div>
...
def _render_delivery_cards(my_rows: list[dict], done: dict) -> None:
    ...
    for i, row in enumerate(my_rows):
        ...
        if is_done:
            border, bg, badge, badge_color = "2px solid #16a34a", "#f0fdf4", "✅ 완료",   "#16a34a"
        elif is_late:
            border, bg, badge, badge_color = "2px solid #dc2626", "#fff5f5", "⚠️ 지연",  "#dc2626"
        else:
            border, bg, badge, badge_color = "2px solid #3b82f6", "#eff6ff", f"#{i+1} 대기", "#3b82f6"

        st.markdown(f"""
<div style="border:{border};border-radius:10px;padding:14px 16px;
            margin-bottom:10px;background:{bg};">
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
    <span style="font-size:1.1rem;font-weight:700;">{row.get('거점','')}</span>
    <span style="background:{badge_color};color:white;border-radius:12px;
                 padding:2px 10px;font-size:0.8rem;font-weight:600;">{badge}</span>
  </div>
  <div style="color:#555;font-size:0.9rem;line-height:1.8;">
    🕐 예상 도착: <b>{eta_txt}</b> &nbsp; 약속 시간: <b>{tw}</b>
    {"&nbsp;<span style='color:#dc2626;font-weight:600;'>" + margin_txt + "</span>" if margin_txt else ""}<br>
    🛣️ {dist_t} &nbsp;|&nbsp; 📝 {memo_t}
  </div>
</div>
""", unsafe_allow_html=True)
