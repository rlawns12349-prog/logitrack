...
개선 사항:
  - _conn(): 커넥션 상태 검사(closed 속성) 후 재연결 시도 추가
  - purge_old_route_cache: SQL 인젝션 방어를 위해 f-string 대신 파라미터 바인딩 사용
  - load_locations / list_scenarios: 반환 타입 명시
  - 모든 메서드 docstring 정비
"""
...
class DBManager:
    ...
    def __init__(self, db_url: str) -> None:
        ...
        with self._conn() as conn:
            with conn.cursor() as cur:
                ...
                """)
                # 기존 DB에 cfg_data 컬럼이 없을 경우 자동 추가 (마이그레이션)
                cur.execute("""
                    ALTER TABLE scenarios
                    ADD COLUMN IF NOT EXISTS cfg_data TEXT DEFAULT '{}'
                """)
                cur.execute("""
    ...
    def purge_old_route_cache(self, ttl_hours: int = _CACHE_TTL_HOURS) -> None:
        ...
        개선: f-string SQL 대신 파라미터 바인딩 사용 (SQL 인젝션 방어).
